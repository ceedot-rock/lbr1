//! Order-0 rANS. Port of the proven pulsar AN0 coder.
...
pub fn normalize_counts(counts: &[u32; 256]) -> ([u32; 256], [u32; 256]) {
    let total: u32 = counts.iter().sum();
    let mut freq = [0u32; 256];
    if total == 0 {
        freq[0] = SCALE;
        return (freq, [0u32; 256]);
    }
...
pub fn rans_encode(data: &[u8]) -> Vec<u8> {
    ...
    // breakpoint: normalize (snapshot / residual)
    let (freq, start) = normalize_counts(&counts);
    {
        let mut sent = crate::sentinel::Sentinel::new();
        let obs = crate::sentinel::Sentinel::observe_stream(data, data.len() / 2);
        let _tick = sent.step(obs, obs[0], 0.0);
    }
...
pub fn rans_decode(buf: &[u8]) -> Result<Vec<u8>, &'static str> {
    ...
    if run != SCALE {
        let mut sent = crate::sentinel::Sentinel::new();
        let tick = sent.step([0.0, 1.0, 0.0, 0.0, 1.0], 1.0, 1.0);
        let _ = crate::sentinel::decode_breakpoint(&tick, false);
    ...
    for i in 0..orig_len {
        ...
        while state < RANS_L {
            if cursor == 0 {
                let mut sent = crate::sentinel::Sentinel::new();
                let tick = sent.step([0.0, 1.0, 0.0, 1.0, 0.0], 1.0, 1.0);
                let _ = crate::sentinel::decode_breakpoint(&tick, false);
...
mod tests {
    ...
    fn roundtrip_prose() {
        ...
        let e = rans_encode(s);
        assert_eq!(rans_decode(&e).unwrap(), s);
    }
    #[test]
    fn roundtrip_binary() {
        let s: Vec<u8> = (0..400u32).map(|i| ((i * 37 + 11) % 256) as u8).collect();
        assert_eq!(rans_decode(&rans_encode(&s)).unwrap(), s);
    }
}
