#!/usr/bin/env bash
# PCC Dial C FAIL_LOUD harness — prefer HASH=16 INSERT=ends; exit 2 if packed >= zstd-9 or DECODE_OK false.
set -euo pipefail
...
export LBR1_HASH="${LBR1_HASH:-16}"
export LBR1_INSERT="${LBR1_INSERT:-ends}"
...
if [[ $RC -ne 0 ]] || echo "$OUT" | grep -q 'FAIL_LOUD=true'; then
  echo "FAIL_LOUD gate tripped (Dial C prefer)" >&2
  exit 2
fi
...
echo "Dial C PASS vs zstd-9 (pack_bytes=$PACK < $ZSTD9) — unexpected; re-check lead vs Dial A"
