#!/usr/bin/env bash
# PCC Dial B FAIL_LOUD harness — exit 2 if packed >= zstd-9 or DECODE_OK false.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
MOZ="${1:-${MOZILLA:-/workspace/corpora/silesia-members/mozilla}}"
ZSTD9=16735963
export LBR1_PARSE="${LBR1_PARSE:-hc4}"
export LBR1_WINDOW="${LBR1_WINDOW:-524288}"
...
echo "Dial B PASS vs zstd-9 (pack_bytes=$PACK < $ZSTD9)"
