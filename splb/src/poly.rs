//! AWARE pack v1 — lossless MDL polyfit peel (`poly_d1` + zero-residual flag).
//!
...
//! Header for poly_d1 zero-residual = 1+1+16+1 = 19 B (CI ≤ 21).
...
pub const MODEL_AFFINE_I32: u8 = 3; // reserved — not emitted in v1
pub const MODEL_POLY_D3: u8 = 4;

/// Pack v1 header size for deg-1 zero-residual (basement).
...
/// MDL pick among poly deg 0..=3 (affine reserved). Returns (inner, model_id, deg).
...
mod tests {
    ...
    fn int_ramp_256k_leq_21_roundtrip() {
        ...
        assert_eq!(tag, "poly_d1");
        let ab = aware_bytes(&blob).expect("aware_bytes");
        assert!(
            ab <= 21,
            "aware_bytes={ab} want ≤21 (pack v1 header 19 B)"
        );
        assert_eq!(ab, POLY_D1_ZERO_HEADER, "zero-residual poly_d1 must be 19 B");
    ...
    fn simple_ramp_small() {
        ...
        assert_eq!(tag, "poly_d1");
        assert_eq!(decode(&blob).unwrap(), raw);
        assert!(aware_bytes(&blob).unwrap() <= 21);
    }

    #[test]
    fn encode_best_picks_poly_d1() {
        let raw = int_ramp_256k();
    ...
    fn noisy_ramp_roundtrip_with_residuals() {
        ...
        assert_eq!(decode(&blob).unwrap(), raw);
        // Non-zero residuals → flag 0 → larger than 19 B header.
        assert!(aware_bytes(&blob).unwrap() > POLY_D1_ZERO_HEADER);
    }
}
